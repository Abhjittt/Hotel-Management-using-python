...
def recep_log():
    ...
    print()
    print('-----RECEPTION LOGIN------')
    a=input('Enter your UserID:')
    li = rec_log_data['USERID'].tolist()
    if a in li:
        b=input('Enter your Password:')
        print('Checking...')
        che = int(a[-1])
        if b == rec_log_data.loc[che-1,'PASSWORD']:
    ...
    if(a == 0):
        for i in range(0,20):
            if ALL.loc[i,'STATUS'] == 'UNOCCUPIED':
                print("customer's room number is",i+1)
                typ=ALL.loc[i,'TYPE']
        ...
        for i in range(50,55):
            if ALL.loc[i,'STATUS'] == 'UNOCCUPIED':
                print('customers room number is',i+1)
                typ=ALL.loc[i,'TYPE']
                ALL.loc[i,'STATUS']='OCCUPIED'
                ALL.loc[i,'CUSTOMER NAME']=name
                ALL.loc[i,'MOBILE NUM']=pn
...
def recep():
    ...
    if choice == 1:
        ...
        roomno=int(input('enter customers room number'))
        ALL=pd.read_csv('ALL.csv')
        print("-------------------------------------------------------------------------------------") 
        print("                                 Hotel SPECTRA") 
        print("-------------------------------------------------------------------------------------") 
        ...
        while x==1:
            ...
            tp = price*amt
            b=input('enter y if u want to add more to the customers order')
            if b == 'y':
                x=1
                TP=TP+tp
            else:
        ...
        if ALL.loc[roomno-1,'STATUS'] == 'UNOCCUPIED':
            print('PLEASE ENTER THE CORRECT ROOM NUMBER')
             ...
             msg = EmailMessage()
             msg['Subject']='Checked out from Hotel SPECTRA'
             msg['From']='reservations.spectra@gmail.com'
             msg['To']=ema
        ...
        if custid == ALL.loc[roomno-1,'CUSTID']:
            ...
            if ALL.loc[roomno-1,'REST COST']==0:        
                ...
                print('YOU CANT CANCEL THE BOOKING')
                recep()
